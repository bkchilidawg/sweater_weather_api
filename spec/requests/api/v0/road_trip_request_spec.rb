...
RSpec.describe "Road Trip Request" do
  ...
  it "Post request for road_trip show", :vcr do
    ...
    expect(data[:weather_at_eta][:daily_weather][1][:date]).to be_a String
    expect(data[:weather_at_eta][:daily_weather][1][:max_temp]).to be_a Float
    expect(data[:weather_at_eta][:daily_weather][1][:condition]).to be_a String
  end
  ...
  it "Post request for road_trip show w/bad API key", :vcr do
    ...
    expect(json_response[:error][:message][:message]).to eq("Invalid API key")
  end
